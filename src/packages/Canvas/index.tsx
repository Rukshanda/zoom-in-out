...
class Canvas<T> extends Component<CanvasProps<T>> {
  ...
  private draw = (props = this.props) => {
    ...
    this.ctx.save();
    this.ctx.fillStyle = config.backgroundColor || BACKGROUND_COLOR;
  ...
  /* ------ WHEEL EVENT HANDLE ------ */
  private onWheel = (e: WheelEvent, isAuto?: boolean) => {
    e.preventDefault?.();
    ...
    const newRx = zoomedX_INV(mouse.x, this.zoom);
    const newRy = zoomedY_INV(mouse.y, this.zoom);

    /* ------ AUTO SCROLL WHEN MOVE OUT ------ */
    if (isAuto) {
      const { getControl } = this.props;
      const control = getControl();
      control.onMouseMoveAuto({
        pageX: mouse.rx - newRx,
        pageY: mouse.ry - newRy,
      });
    }
    /* ------ AUTO SCROLL WHEN MOVE OUT ------ */
  ...
  private onMouseMove = (event: MouseEvent) => {
    this.onMouseEvent(event);
    const { getControl, windowSize } = this.props;
    const { mouseDrag, onMouseMove } = getControl();

    /* ------ AUTO SCROLL WHEN MOVE OUT ------ */
    const { xScrollAuto, yScrollAuto } = checkAutoScroll(event, windowSize);
    this.app.autoScroll =
      !!mouseDrag && (xScrollAuto !== 0 || yScrollAuto !== 0);
    if (this.app.autoScroll) {
      if (this.frameScroll) window.cancelAnimationFrame(this.frameScroll);
      this.autoScroll(xScrollAuto, yScrollAuto);
    }
    /* ------ AUTO SCROLL WHEN MOVE OUT ------ */
    onMouseMove(event);
    this.draw();
  ...
  private autoScroll = (x: number, y: number) => {
    if (!this.app.autoScroll) return;
    for (let i = 0; i < 30 / INIT_SCALE; i++) {
      this.onWheel({ deltaX: x, deltaY: y } as WheelEvent, true);
    }
    this.frameScroll = window.requestAnimationFrame(() =>
      this.autoScroll(x, y)
    );
  };
  /* ------ AUTO SCROLL WHEN MOVE OUT ------ */
